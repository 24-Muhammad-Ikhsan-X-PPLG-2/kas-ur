  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="grid grid-cols-2 gap-3">
            {(["weekly", "monthly"] as const).map((period) => (
              <label key={period} className="cursor-pointer">
                ...
                />
                <span className="block rounded-md border-[3px] border-[#241a1a] bg-white p-3 text-center text-sm font-black peer-checked:bg-[#550000] peer-checked:text-[#fffaf2] peer-focus-visible:outline-3 peer-focus-visible:outline-[#550000]">
        ...
        <label className="flex cursor-pointer items-center justify-between rounded-md border-2 border-[#241a1a] p-3">
          <span>
            <span className="block text-sm font-extrabold">Status</span>
            <span className="text-xs text-[#6f6262]">Konfigurasi Aktif</span>
          </span>
          <input
            type="checkbox"
            className="h-5 w-5 accent-[#550000]"
            {...configForm.register("isActive")}
          />
        </label>
        <div className="flex items-start gap-3 border-2 border-[#241a1a] bg-[#f4e4df] p-3 text-xs leading-relaxed">
          <AlertTriangle size={18} className="mt-0.5 shrink-0 text-[#550000]" />{" "}
          <span>
